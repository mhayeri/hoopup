...
export default function HomeSessionCard({ entry, now, live = false }: Props) {
  ...
  return (
    ...
    >
      ...
      {live ? (
        <span className="inline-flex w-fit items-center gap-1.5 rounded-full bg-emerald-500 px-2 py-0.5 text-[11px] font-bold tracking-wide text-white uppercase">
      ...
      <div className="flex items-center gap-2 text-xs">
        <div className="flex h-6 w-6 shrink-0 items-center justify-center overflow-hidden rounded-full border border-[var(--color-court)]/20 bg-[var(--color-net)]">
          {host?.avatar_url ? (
            ...
            <span className="text-[10px] font-bold text-[var(--color-hardwood)]/60 uppercase">
              {initial}
            </span>
          )}
        </div>
        <span className="truncate font-semibold text-[var(--color-ink)]/80">
          {host ? `@${host.username}` : 'Unknown host'}
        </span>
        {host?.skill_level ? (
          <span className="rounded-full bg-[var(--color-hardwood)]/10 px-1.5 py-0.5 text-[9px] font-bold tracking-wide text-[var(--color-hardwood)] uppercase">
            {host.skill_level}
          </span>
        ) : null}
