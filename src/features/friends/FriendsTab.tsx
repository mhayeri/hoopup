    ...
    <div className="space-y-8">
      {isSelf && incoming.length > 0 ? (
        <section>
          ...
          <div className="mt-3 rounded-2xl border border-[var(--color-court)]/30 bg-[var(--color-court)]/5 p-3">
      ...
      {isSelf && outgoing.length > 0 ? (
        <section>
          <details className="rounded-2xl border border-[var(--color-ink)]/10 p-3">
            <summary className="flex cursor-pointer items-center gap-3 text-xs font-bold uppercase tracking-[0.18em] text-[var(--color-hardwood)]">
              <span>Sent requests</span>
              <span className="rounded-full bg-[var(--color-ink)]/5 px-2 py-0.5 text-[10px] font-bold normal-case tracking-normal text-[var(--color-ink)]/70">
...
function SectionHead({ label, count, suffix }: { label: string; count: number; suffix?: string }) {
  return (
    <div className="flex items-center gap-3">
      <span className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--color-hardwood)]">
        {label}
      </span>
      <span className="rounded-full bg-[var(--color-court)]/15 px-2 py-0.5 text-xs font-bold text-[var(--color-court)]">
        {count}
        {suffix ? ` ${suffix}` : ''}
      </span>
    </div>
...
function EmptyState({ isSelf }: { isSelf: boolean }) {
  return (
    <div className="mt-3 rounded-2xl border border-dashed border-[var(--color-ink)]/20 p-8 text-center">
